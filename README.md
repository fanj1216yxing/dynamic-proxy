...
- 🧩 **主流订阅识别**: 混合入口可识别 `http/https/socks5/socks5h/vmess(vmiss)/vless(vliss)/hy2` 节点
...
proxy_list_urls:
  ...
  # 也支持 Clash 订阅（YAML），混合入口可自动识别 http/https/socks5/vmess/vless/hy2 节点
  # 添加更多源
...
> 说明：`vmess/vless/hy2`（含 `vmiss/vliss` 拼写兼容）会被混合池识别并参与健康检查与 CF 可通过性筛选。
### 使用方法
