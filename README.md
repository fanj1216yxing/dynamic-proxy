...
# Force rotate to next proxy (both strict/relaxed pools, this is a normal HTTP endpoint)
curl http://127.0.0.1:17287
# If auth is enabled
curl -u username:password http://127.0.0.1:17287
```
...
# 强制轮换到下一个代理（strict/relaxed 池都会切换，这是普通 HTTP 端点）
curl http://127.0.0.1:17287
# 如果开启认证
curl -u username:password http://127.0.0.1:17287
```
