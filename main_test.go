...
func TestParseSpecialProxyURLMixed_ExtractsDecoratedURI(t *testing.T) {
	content := strings.Join([]string{
		"node-a => vless://uuid-abc@10.0.0.1:443?security=tls#tag ]],",
		"random text trojan://token@10.0.0.2:8443);",
	}, "\n")
...
func TestParseSpecialProxyURLMixed_ExtractsVMESSBase64URI(t *testing.T) {
	payload := `{"add":"11.22.33.44","port":"443"}`
	entry := "vmess://" + base64.StdEncoding.EncodeToString([]byte(payload))
	content := "[HK] " + entry + " ]"

	proxies := parseSpecialProxyURLMixed(content)
	if len(proxies) != 1 {
		t.Fatalf("expected 1 parsed proxy, got %d (%v)", len(proxies), proxies)
	}
	if proxies[0] != "vmess://11.22.33.44:443" {
		t.Fatalf("unexpected vmess normalization result: %s", proxies[0])
	}
}
